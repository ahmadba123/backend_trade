...
class Trade {
    ...
    static async findAll(limit, offset, query) {
        let sql = "SELECT * FROM mt5_deals where Deal like ? OR Login Like ?  order by Deal ASC LIMIT ?, ?";
        query = query ? query : "";

        limit = Number(limit) >= 0 ? Number(limit) : 10;
        offset = Number(offset) >= 0 ? Number(offset) : 0;

        let results = (await db.query(sql, [query + "%", query + "%", offset, limit])).values();
        return results;
    }

    static async findAllTrade() {
        let sql = "SELECT * FROM mt5_deals";
